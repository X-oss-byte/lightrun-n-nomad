...
var (
	...
	configSpec = hclspec.NewObject(map[string]*hclspec.Spec{
		"lightrun_server":       hclspec.NewAttr("lightrun_server", "string", false),
		"lightrun_secret":       hclspec.NewAttr("lightrun_secret", "string", false),
		"lightrun_certificate": hclspec.NewDefault(
			hclspec.NewAttr("lightrun_certificate", "string", false),
			hclspec.NewLiteral(`"ee80811b38e7e6c2dc4cc372cbea86bd86b446b012e427f2e19bf094afba5d12"`),
		),
	})
	...
	taskConfigSpec = hclspec.NewObject(map[string]*hclspec.Spec{
		...
		"lightrun_server":    hclspec.NewAttr("lightrun_server", "string", false),
		"lightrun_secret":    hclspec.NewAttr("lightrun_secret", "string", false),
		"lightrun_certificate":    hclspec.NewAttr("lightrun_certificate", "string", false),
	})
...
type Config struct {
	LightrunUrl    string   `codec:"lightrun_server"`
	LightrunSecret string   `codec:"lightrun_secret"`
	LightrunCert   string   `codec:"lightrun_certificate"`
}
...
func (d *Driver) StartTask(cfg *drivers.TaskConfig) (*drivers.TaskHandle, *drivers.DriverNetwork, error) {
	...
	lightrunSoPath, err := d.downloadAndExtractLightrunSoIfNeeded(cfg)

	if err != nil {
		return nil, nil, fmt.Errorf("failed to download lightrun zip: %s", err)
	}

	args := d.javaCmdArgs(driverConfig, lightrunSoPath)
...
func (d *Driver) downloadAndExtractLightrunSoIfNeeded(cfg *drivers.TaskConfig) (string, error) {

	lightrunSoLocalPath := "/local/lightrun_agent.so"
	soPath := cfg.TaskDir().Dir + lightrunSoLocalPath

	d.logger.Info("Checking for lightrun so at", "so-path", soPath)
	_, err := os.Stat(soPath)
	if err == nil {
		d.logger.Info("so already downloaded at", "so-path", soPath)
		return lightrunSoLocalPath, nil
	}

	d.logger.Info("so file was not found")
	if !os.IsNotExist(err) {
		d.logger.Error("error getting so information", "err", err)
		return "", fmt.Errorf("unable to determine whether lightrun's so exists on path %s", soPath)
	}
	
	agentZipFile := cfg.TaskDir().Dir + "/local/agent.zip"
	if _, err := os.Stat(agentZipFile); err != nil || os.IsNotExist(err) {
		...
		d.logger.Debug("status", resp.Status)

		if resp.StatusCode != 200 {
			return "", fmt.Errorf("failed to download lightrun zip, error: %s", resp.Body)
		}

		out, err := os.Create(agentZipFile)
		if err != nil {
		...
		if err != nil {
			...
			d.logger.Info("Download Done", agentZipFile)
		}
	}

	d.logger.Info("Unzipping", agentZipFile)
	Unzip(agentZipFile, cfg.TaskDir().Dir + "/local")
	d.logger.Info("Unzipping Done", agentZipFile)
	
	return lightrunSoLocalPath, nil
}

func (d *Driver) javaCmdArgs(driverConfig TaskConfig, lightrunSoPath string) []string {
	d.logger.Info("using lightrun so from", "so-path", lightrunSoPath)

	var serverUrl = driverConfig.LightrunUrl
	var serverCert = driverConfig.LightrunCert
	var secret = driverConfig.LightrunSecret

	if serverUrl == "" {
		serverUrl = d.config.LightrunUrl
	}
	if serverCert == "" {
		serverCert = d.config.LightrunCert
	}
	if secret == "" {
		secret = d.config.LightrunSecret
	}
	
	args := []string{}

	args = append(args, "-agentpath:" + lightrunSoPath, "-Dcom.lightrun.server="+serverUrl, "-Dcom.lightrun.secret="+secret,
		"-Dpinned_certs="+serverCert, "-Dtransmission_bulk_max_size=10")
